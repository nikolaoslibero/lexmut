...
#[expect(clippy::too_many_lines, reason = "PAIRS vec is very long.")]
fn swap_character(char: char) -> char {
    const PAIRS: [(char, char); 52] = [
        ('a', 'e'),
        ('e', 'i'),
        ('i', 'o'),
        ('o', 'u'),
    ...
    PAIRS
        .iter()
        .find(|&&(input, _)| input == char)
        .map_or(char, |&(_, output)| output)
}
